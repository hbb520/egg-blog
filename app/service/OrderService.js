...
module.exports = app => {
  ...
  return class OrderService extends Service {
    ...
    async alipayCallback(body) {
      ...
      if (tradeStatus === AlipayConst.TRADE_SUCCESS) await orderRow.update({
    ...
    async cancel(orderNum) {
      ...
      const updateRow = await orderRow.update({
        status: ORDER_STATUS_MAP.CANCELED.CODE,
        closeTime: new Date()
      }, {individualHooks: true});
    ...
    async getOrderCartProduct() {
      ...
      return this.ServerResponse.createBySuccessMsgAndData('订单快照', {
        orderItemArr,
        orderTotalPrice,
        host: 'localhost:7071'
      });
    ...
    async getDetail(orderNum) {
      ...
      const order = await this.OrderModel.findOne({
        where: {
          orderNum,
          userId: role === ROLE_ADMAIN ? {$regexp: '[0-9a-zA-Z]'} : userId
        }
      }).then(r => r && r.toJSON());
    ...
    async manageSendGood(orderNum) {
      ...
      const updateRow = await orderRow.update({
        status: ORDER_STATUS_MAP.SHIPPED.CODE,
        sendTime: new Date()
      }, {individualHooks: true});
    ...
    async _createOrderDetail(order, orderItemList, shippingId) {
      ...
      return {
        order: {...order, payment: Number(order.payment).toFixed(2), paymentType, orderStatus},
        orderItemList,
        shipping,
        host: 'localhost:7071'
      };
    ...
    alipayData(order) {
      let tradeNo = order.orderNum,
        ...
        timeExpress = `50`;
      return {tradeNo, subject, totalAmount, body, timeExpress};
    }
